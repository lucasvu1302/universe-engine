        ...
        <div className="flex items-center space-x-3 pointer-events-auto bg-black/70 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 shadow-xl">
          ...
          <div>
            <div className="text-xs text-slate-400 font-medium">
              {isMars ? t('surface.marsJezeroTitle') : t('surface.moonApolloTitle')}
            </div>
            <div className="text-sm md:text-base font-bold text-white tracking-wider">
              {isMars ? t('surface.marsJezeroSub') : t('surface.moonApolloSub')}
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 w-full">
        <div className="flex items-center space-x-4 pointer-events-auto bg-black/70 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-white/10 text-xs">
          <div className="flex items-center space-x-1.5 text-slate-300">
            ...
            <span>{t('surface.altitude')}: <strong className="text-white">2.4 m</strong></span>
          </div>
          <div className="flex items-center space-x-1.5 text-slate-300">
            <Wind className="w-3.5 h-3.5 text-amber-400" />
            <span>{t('surface.atmosphericPressure')}: <strong className="text-white">{isMars ? '0.006 atm (CO₂)' : '0.000 atm (Vacuum)'}</strong></span>
          </div>
          <div className="flex items-center space-x-1.5 text-slate-300">
            <Thermometer className="w-3.5 h-3.5 text-rose-400" />
            <span>{t('surface.surfaceTemp')}: <strong className="text-white">{isMars ? '-63 °C' : '-20 °C'}</strong></span>
          </div>
          <div className="flex items-center space-x-1.5 text-slate-300">
            <Compass className="w-3.5 h-3.5 text-emerald-400" />
            <span>{t('targetHud.gravity')}: <strong className="text-white">{isMars ? '0.38 g' : '0.16 g'}</strong></span>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center space-x-2 text-[11px] text-slate-400 bg-black/40 px-3 py-1.5 rounded-xl border border-white/5">
          ...
          <span>{t('common.status')}: NOMINAL</span>
        </div>
      </div>
    </div>
  );
